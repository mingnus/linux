...
struct cache {
	...
	/*
	 * Cache_size entries. Set bits indicate out-of-bound mappings for
	 * invalidation.
	 */
...
{
	if (!cache->loaded_discards && get_cache_mode(cache) != CM_WRITE) {
		DMERR("%s: unable to resume a failed-loaded cache, "
		      "please check/repair metadata.", cache_device_name(cache));
		return false;
	}

	if (cache->sized && !cache->loaded_mappings) {
		DMERR("%s: unable to resume cache due to missing cache table reload",
		      cache_device_name(cache));
		return false;
	}
