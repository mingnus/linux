                            ...
                            struct remove_result *res)
{
	...
	uint32_t nr_entries = le32_to_cpu(n->header.nr_entries);
	struct remove_result res1, res2;
	...
	for (; i < nr_entries; i++) {
		...
		if (i == (nr_entries - 1)) {
			...
			remove_(tm, data_sm, child,
                                thin_begin, thin_end, &res2);

			/* Update the child pointer */
			if (res2.nr_nodes == 1) {
				if (res2.nodes[0].nr_entries == 0) {
					erase_internal_entry(n, i);
					i -= 1;
					nr_entries -= 1;
					dm_tm_dec(tm, res2.nodes[0].loc);
				} else {
					n->values[i] = cpu_to_le64(res2.nodes[0].loc);
				}
			}
		} else {
			...
			if (key >= thin_begin && next_key <= thin_end) {
				...
				remove_(tm, data_sm, child,
	                                thin_begin, thin_end, &res1);

				/* Update the child pointer */
				if (res1.nr_nodes == 1) {
					if (res1.nodes[0].nr_entries == 0) {
						erase_internal_entry(n, i);
						i -= 1;
						nr_entries -= 1;
						dm_tm_dec(tm, res1.nodes[0].loc);
					} else {
						n->values[i] = cpu_to_le64(res1.nodes[0].loc);
					}
				}
			}
		}
	...
	if (res1.nr_nodes <= 1 && res2.nr_nodes <= 1) {
		// FIXME: rebalance the children if they are internal nodes?
		...
		if (res1.nr_nodes == 2) {
			new_node = &res1.nodes[1];
			i -= 1;
		} else {
			new_node = &res2.nodes[1];
		}
		...
		if (nr_entries < INTERNAL_NR_ENTRIES) {
			...
			insert_into_internal(n2, i, res->nodes[1].lowest_key, res->nodes[1].loc);
