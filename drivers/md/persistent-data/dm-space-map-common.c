...
static int sm_ll_inc_overflow_(struct ll_disk *ll, dm_block_t b, struct inc_context *ic)
{
	int r;
	int index = -1;
	struct btree_node *n;
...
static int sm_ll_inc_overflow(struct ll_disk *ll, dm_block_t b, struct inc_context *ic)
{
	...
	return sm_ll_inc_overflow_(ll, b, ic);
}
...
static int sm_ll_del_overflow_(struct ll_disk *ll, dm_block_t b,
                               struct inc_context *ic)
{
	reset_inc_context(ll, ic);
...
static int sm_ll_dec_overflow_(struct ll_disk *ll, dm_block_t b,
                               struct inc_context *ic, uint32_t *old_rc)
{
	...
	n = dm_block_data(ic->overflow_leaf);
        ...
        *old_rc = rc;
        ...
        if (rc == 3) {
	        return sm_ll_del_overflow_(ll, b, ic);
        } else {
	        rc--;
	        *v_ptr = cpu_to_le32(rc);
	        return 0;
                                   ...
                                   struct inc_context *ic, uint32_t *old_rc)
{
	...
	return sm_ll_dec_overflow_(ll, b, ic, old_rc);
}
