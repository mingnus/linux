...
struct dm_pool_metadata {
	...
	/*
	 * Set if error happened during the heartbeat process. Possibly caused
	 * by a failure io, or sequence mismatch generated by multiple activation.
	 */
	bool fail_heartbeat:1;
...
int __test_and_set_heartbeat_block(struct dm_pool_metadata *pmd)
{
	struct heartbeat_info hb_next;
	int r;

	r = __test_heartbeat_block(pmd, &hb_next);
	if (r)
		goto bad_fail_heartbeat;
...
int dm_pool_test_and_set_heartbeat_block(struct dm_pool_metadata *pmd)
{
	...
	/* FIXME: use another semaphore to avoid performance impact? */
	pmd_write_lock_in_core(pmd);
	memcpy(&pmd->hb_info, &hb_next, sizeof(hb_next));
...
static int __cleanup_heartbeat_block(struct dm_pool_metadata *pmd)
{
	struct heartbeat_info hb_next;
	int r;
...
int dm_pool_metadata_close(struct dm_pool_metadata *pmd)
{
	...
	if (!pmd->fail_heartbeat)
		__cleanup_heartbeat_block(pmd);
